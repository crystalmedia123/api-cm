...
module.exports = {
    async createTransaction(req,res){
        try {
            ...
            if (!data.owner) return res.status(400).send({"error":"owner is required"});
            if (!data.coinAddress) return res.status(400).send({"error":"coinAddress is required"});
            if (!data.transactionType) return res.status(400).send({"error":"transactionType is required"});
            if (!data.amount) return res.status(400).send({"error":"amount is required"});
            ...
            await transaction.save((err,doc)=>{
                if (!err){
                    return res.status(200).send({"success":"Transaction awaiting comfirmation"});
                }
                else{
    ...
    async updateTransaction(req,res){
        try {
            ...
            if (!transaction) return res.status(404).send({"error":"User not found"});

            transaction.transactionStatus = data.transactionStatus;

            await transaction.save((err,doc)=>{
                if (!err){
                    return res.status(200).send({"success":"Transaction status updated"});
                }
                else{
                    return res.status(400).send({"error":err});
                }
    ...
    async deleteTransaction(req,res){
        try {
            ...
            if(!coin) return res.status(200).send({"error": "transaction not found"});
            try {
                coin.remove((err, doc)=>{
                    if (!err){
                        if (doc.image){
